...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
        ...
        //beanFactoryPostProcessors这个自身为0，因为他自身没有加什么东西
		// Invoke BeanDefinitionRegistryPostProcessors first, if any.
		Set<String> processedBeans = new HashSet<>();

		//何为注册器，就是把我们扫描出来的bd放入bdmap中，本身的大环境就是一个注册器，工厂其实也是一个注册器，所以需要等于这个工厂
		if (beanFactory instanceof BeanDefinitionRegistry) {
			BeanDefinitionRegistry registry = (BeanDefinitionRegistry) beanFactory;

			//这里为什么新建两个列表，并且一个BeanDefinitionRegistryPostProcessor还是继承BeanFactoryPostProcessor，肯定有其独到的方法需要用
			List<BeanFactoryPostProcessor> regularPostProcessors = new ArrayList<>();
			List<BeanDefinitionRegistryPostProcessor> registryProcessors = new ArrayList<>();

			//自定义的beanFactoryPostProcessors，如果没有加自定义的，那么下面的for循环其实都不用看了的
			...
			//这个currentRegistryProcessors 放的是spring内部自己实现了BeanDefinitionRegistryPostProcessor接口的对象，上面的是我们自己写的
			...
			//bean工厂中根据bean的类型获取到相应的bean的名字，这里面只有一个，那就是那个configuration那个
